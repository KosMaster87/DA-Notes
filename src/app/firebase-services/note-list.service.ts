...
export class NoteListService implements OnDestroy {
  ...
  subNotesListMarked() {
    const notesQuery = query(this.getNotesRef(), where('marked', '==', true), limit(100));
    return onSnapshot(notesQuery, (list) => {
      this.normalNotes_Marked = [];
      list.forEach((element) => {
        this.normalNotes_Marked.push(this.setNoteObject(element.data(), element.id));
      });
    });
  }
  ...
  async updateNote(note: Note) {
    ...
    if (note.id && colId) {
      const docRef = this.getSingelDocRef(colId, note.id);
      ...
      if (docSnapshot.exists()) {
        ...
        console.error(`Dokument mit ID ${note.id} in Sammlung ${colId} existiert nicht`);
      }
    }
  }
  ...
  getCleanJson(note: Note): Partial<Note> {
    return {
      type: note.type,
      title: note.title,
